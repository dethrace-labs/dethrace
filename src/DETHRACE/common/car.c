...
tS3_sound_tag gSkid_tag[2] = { 0 };
tCar_spec* gLast_car_to_skid[2] = { 0 };
int gEliminate_faces = 0;
br_vector3 gZero_v__car = { 0 }; // suffix added to avoid duplicate symbol
tU32 gSwitch_time = 0;
tSave_camera gSave_camera[2] = { 0 };
tU32 gLast_mechanics_time = 0;
int gOpponent_viewing_mode = 0;
...
void SetCarSuspGiveAndHeight(tCar_spec* pCar, br_scalar pFront_give_factor, br_scalar pRear_give_factor, br_scalar pDamping_factor, br_scalar pExtra_front_height, br_scalar pExtra_rear_height) {
    ...
    pCar->sk[0] = pCar->M / (ratio + 1.f) * 5.f / rear_give;
    pCar->sb[0] = pCar->M / (ratio + 1.f) * sqrtf(5.f) / sqrt(rear_give);
    ratio = 1.f / ratio;
    pCar->sk[1] = pCar->M / (ratio + 1.0) * 5.f / front_give;
    pCar->sb[1] = pCar->M / (ratio + 1.0) * sqrtf(5.f) / sqrt(front_give);
